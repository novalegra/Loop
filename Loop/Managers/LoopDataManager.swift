...
final class LoopDataManager {
    ...
    fileprivate var recommendedSMBTempBasal: (recommendation: TempBasalRecommendation, date: Date)?
    
    fileprivate var recommendedSMB: (recommendation: BolusRecommendation, date: Date)?
...
extension LoopDataManager {
    ...
    func bolusConfirmed(_ dose: DoseEntry, completion: (() -> Void)?) {
        self.dataAccessQueue.async {
            ...
            self.recommendedSMBTempBasal = nil
            self.insulinEffect = nil
            self.insulinOnBoard = nil
            self.notify(forChange: .bolus)
        ...
        if insulinOnBoard == nil {
            ...
            doseStore.getInsulinOnBoardValues(start: Date() - TimeInterval(minutes: 5)) { (result) in
                ...
                case .success(let value):
                    self.insulinOnBoard = value.closestPrior(to: Date())
                }
                updateGroup.leave()
            }
        }
    ...
    private func updatePredictedGlucoseAndRecommendedBasalAndBolus() throws {
        ...
        var enableSMB = false
        // only enable SMB if explicitly enabled in the settings
        // TODO: add enableSMBWithCarbs
        if let carbsOnBoard = carbsOnBoard, carbsOnBoard.quantity.doubleValue(for: HKUnit.gram()) > 0, settings.enableSMBWithCOB {
            enableSMB = true
        } else if settings.enableSMBAlways {
            enableSMB = true
        }
        ...
        if enableSMB, let carbRatios = carbRatioScheduleApplyingOverrideHistory {
            ...
            if let minIOBPredBG = IOBPredBG, let iob = insulinOnBoard?.value {
                let recommendation = predictedGlucose.recommendedSuperMicrobolus(
                    ...
                    sensitivities: insulinSensitivity,
                    model: model,
                ...
                if let tempBasal = recommendation?.0, let smb = recommendation?.1 {
                    ...
                    self.logger.default("Recommending zero-ish temp basal: \(tempBasal) at \(startDate)")
                    self.logger.debug("Recommending super-microbolus: \(String(describing: smb))")
    ...
    private func setRecommendedTempBasal(_ completion: @escaping (_ error: Error?) -> Void) {
        ...
        guard let recommendedTempBasal = self.recommendedSMBTempBasal ?? self.recommendedTempBasal else {
            completion(nil)
            return
        }
